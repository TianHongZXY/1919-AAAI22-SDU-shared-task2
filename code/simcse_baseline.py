...
def predict(model, data, diction):
    predictions = []
    for d in data:
        pred = {
            'ID': d['ID'],
        ...
        highest_score = -1
        best = ''
        inputs = tokenizer(sent, truncation=True, return_tensors="pt")
        with torch.no_grad():
            embeddings = model(**inputs, output_hidden_states=True, return_dict=True).pooler_output

        for candid in candids:
            score = 0
            cand_inputs = tokenizer(candid, truncation=True, return_tensors="pt")
            cand_emb = model(**cand_inputs, output_hidden_states=True, return_dict=True).pooler_output
            cosine_sim_0_1 = 1 - cosine(embeddings[0], embeddings[1])
            cosine_sim_0_2 = 1 - cosine(embeddings[0], embeddings[2])
            score = 1 - cosine(embeddings, cand_emb)
...
if __name__ == "__main__":
    ...
    predictions = predict(model, data, diction)
